...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QTextBrowser, QFileDialog, QTextEdit, QComboBox, QPushButton, QMessageBox, QFrame, QInputDialog, QLabel, QCheckBox, QScrollBar, QDialogButtonBox, QDialog, QGridLayout
import hashlib
...
class DiamondSorter(QtWidgets.QMainWindow):
    ...
    def display_function(self, function_name):
        ...
        self.running_task_placeholder.setText(function_name)
    ...
    def setup_buttons(self):
        ...
        self.startsortingButton.clicked.connect(self.start_sorting)
        self.password_start_sorting.clicked.connect(self.password_sorting)
        ...
        self.start_sortingButton = QPushButton("Start Sorting")
        self.start_sortingButton.clicked.connect(self.start_sorting)
        self.emailPasswordButton = QPushButton("Email:Password")
        self.emailPasswordButton.clicked.connect(self.email_password)
        self.usernamePasswordButton = QPushButton("Username:Password")
        self.usernamePasswordButton.clicked.connect(self.username_password)
        self.memberIDPINButton = QPushButton("Member ID:PIN")
        self.memberIDPINButton.clicked.connect(self.member_id_pin)
        self.numberPasswordButton = QPushButton("Number:Password")
        self.numberPasswordButton.clicked.connect(self.number_password)
        self.wordpressFinderButton = QPushButton("Wordpress Finder")
        self.wordpressFinderButton.clicked.connect(self.wordpress_finder)
        self.businessEmailsButton = QPushButton("Business Emails")
        self.businessEmailsButton.clicked.connect(self.business_emails)
        self.govDomainsButton = QPushButton("Gov Domains")
        self.govDomainsButton.clicked.connect(self.gov_domains)
        self.serverInformationButton = QPushButton("Server Information")
        self.serverInformationButton.clicked.connect(self.server_information)
        self.cpanelAccountsButton = QPushButton("Cpanel Accounts")
        self.cpanelAccountsButton.clicked.connect(self.cpanel_accounts)
        self.emailsButton = QPushButton("Emails")
        self.emailsButton.clicked.connect(self.emails)
        self.checkmarkButton = QPushButton("✅")
        self.checkmarkButton.clicked.connect(self.checkmark)
        self.advertisementsButton = QPushButton("Advertisements")
        self.advertisementsButton.clicked.connect(self.advertisements)
        self.socialsForumsButton = QPushButton("Socials && Forums")
        self.socialsForumsButton.clicked.connect(self.socials_forums)
    ...
    def cpanel_accounts(self):
        # Logic for the "Cpanel Accounts" button
        message_box = QtWidgets.QMessageBox()
        message_box.setText("You are about to launch the built-in browser. Continue?")
        message_box.setWindowTitle("Diamond Sorter - Window")
        message_box.setStandardButtons(QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
    ...
    def open_url_tools_ui(self):
        url_tools_dialog = QtWidgets.QDialog()
        uic.loadUi(r'ui_files\url_tools.py', url_tools_dialog)
        url_tools_dialog.exec_()
