...
class DiamondSorter(QtWidgets.QMainWindow):
    ...
    def handle_scrape_backup_codes(self):
        directory_path = self.directoryResultsTextBox.toPlainText()
    ...
    def handle_telegram_folder_sorting(self, loaded_directory):
        # Confirm the loaded file directory path
        if not os.path.isdir(loaded_directory):
    ...
    def ask_user_dialog_box(self, prompt):
        # Implement your dialog box functionality here
        # Return the user's choice (e.g., "Copy" or "Move")
        pass
        ...
        if reply == QMessageBox.Yes:
            ...
            for root, dirs, files in os.walk(selected_directory):
                for directory in dirs:
                    ...
                    self.console_widget_textedit.appendPlainText(directory_path)  # Display the directory path in the console
    
                for file in files:
                    if file.endswith(".dat"):
                        file_path = os.path.join(root, file)
                        self.console_widget_textedit.appendPlainText(file_path)  # Display the file path in the console
    ...
    def handle_newtextdocuments(self):
        try:
            directory_path = self.directoryResultsTextBox.toPlainText()
            save_directory = self.savedResultsTextBox.toPlainText()
            if directory_path and save_directory:
                count = 0
                destination_folder = None
                subdirectories = []
                for root, dirs, files in os.walk(directory_path):
                    subdirectories.extend(dirs)  # Add subdirectories to the list
    
                # Process subdirectories in intervals of 50
                batch_size = 50
                for i in range(0, len(subdirectories), batch_size):
                    batch_subdirectories = subdirectories[i : i + batch_size]
                    for subdirectory in batch_subdirectories:
                        subdirectory_path = os.path.join(directory_path, subdirectory)
                        for file in os.listdir(subdirectory_path):
                            if file == "New Text Document.txt":
                                count += 1
                                if destination_folder is None:
                                    destination_folder = os.path.join(save_directory, "Diamond Sorter", "New Text Documents", "Date", subdirectory)
                                    os.makedirs(destination_folder, exist_ok=True)
    
                                # Display the message box for copying or moving the file
                                msg_box = QMessageBox()
                                msg_box.setIcon(QMessageBox.Question)
                                msg_box.setWindowTitle("File Action")
                                msg_box.setText(f"Do you want to copy or move the file '{file}'?")
                                msg_box.setStandardButtons(QMessageBox.Copy | QMessageBox.Move)
                                response = msg_box.exec_()
    
                                if response == QMessageBox.Copy:
                                    shutil.copy(os.path.join(subdirectory_path, file), destination_folder)
                                elif response == QMessageBox.Move:
                                    shutil.move(os.path.join(subdirectory_path, file), destination_folder)
                
                message = f"Found {count} occurrences of 'New Text Document.txt'"
                self.console_widget_textedit.appendPlainText(message + '\n')
    
                # Print the saving data message
                save_message = f"Saving Data To {save_directory}"
                self.console_widget_textedit.appendPlainText(save_message + '\n')
    
                # Create the Results.txt file
                results_file_path = os.path.join(save_directory, "Results.txt")
                with open(results_file_path, 'w') as results_file:
                    results_file.write("Results:\n")
                    results_file.write(f"Found {count} occurrences of 'New Text Document.txt'\n")
                    results_file.write(f"Destination folder: {destination_folder}\n")
                
            else:
                # Show an error message informing the user to enter directory paths
                error_message = "Please enter both a directory path and a save directory path."
